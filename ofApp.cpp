...
void ofApp::draw(){
	...
	for (int y = 0; y < this->depth.rows; y += 1) {
		for (int x = 0; x < this->depth.cols; x += 1) {
			int z = this->depth.at<unsigned short>(y, x);
			if (z != 0 && z <768) {
				//ofEllipse(ofVec3f(x, y, z - 1024), 1, 1);
				
				ofPushMatrix();
				ofTranslate(0, 0, z - 768);
			
				this->frame.at<cv::Vec3b>(y, x)[0] = 0;
				this->frame.at<cv::Vec3b>(y, x)[1] = 0;
				this->frame.at<cv::Vec3b>(y, x)[2] = 0;

				ofEllipse(ofVec3f(x, y, 0), 1, 1);
	...
	this->frame_img.draw(-this->frame_img.getWidth() / 2, -this->frame_img.getHeight() / 2, 0);
